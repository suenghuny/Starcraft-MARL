...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        self.health_scaling_factor = health_scaling_factor
    ...
    def get_action_feature(self):
        ...
        # print(enemy_feature.shape,"전")
        enemy_feature = enemy_feature[:, :-1]
        # print(enemy_feature.shape,"후")
        no_attack_actions_one_hot = np.eye(
            self.n_actions_no_attack)  # dimension                      : n_actions_no_attack, n_actions_no_attack

        no_attack_action_dummy_feature = np.zeros([self.n_actions_no_attack,
                                                   self.n_node_features - 1])  # no_attack_action_dummy_feature : n_actions_no_attack, feature_dim
        no_attack_action_feature = np.concatenate([no_attack_actions_one_hot, no_attack_action_dummy_feature],
                                                  axis=1)  # concat dim                     : n_actions_no_attack, n_actions_no_attack + feature_dim
        ...
        attack_action_feature = np.concatenate([attack_action_one_hot_init, enemy_feature],
                                               axis=1)  # concat dim                     : num_enemy, n_actions_no_attack + feature_dim
    ...
    def get_node_feature(self, node_id):
        ...
        if unit.health > 0:
            ...
            if unithistoricinfo.last_pos_x == None:
                ...
                health_1st_moment = -(unit.health - unithistoricinfo.last_health) / (
                            self._step_mul * self.health_scaling_factor)
                ...
                health_2nd_moment = -(health_1st_moment - unithistoricinfo.last_health_1st_moment) / (
                            self._step_mul * self.health_scaling_factor)
                shield_2nd_moment = -(shield_1st_moment - unithistoricinfo.last_shield_1st_moment) / (
                            self._step_mul * self.health_scaling_factor)

            pos_feats[2] = pos_x_1st_moment
            pos_feats[3] = pos_y_1st_moment
            pos_feats[4] = pos_x_2nd_moment
            pos_feats[5] = pos_y_2nd_moment
        ...
        unit_type_feats[self.unit_type_ids.index(str(unit.health_max) + str(unit.shield_max) + str(unit.radius))] = 1
        ...
        node_feature = np.concatenate(
            (move_avail_feats, pos_feats, unit_type_feats, health_and_shield_feats, alliance_feats))
